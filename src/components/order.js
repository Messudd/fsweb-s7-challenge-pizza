...
const submitStatus = () => {
  return (!(select === '- Hamur Kalınlığı Seç -') && radioState.length>0 && pername.length>4 && orderCount<11 && (orderNote.length<50 && orderNote.length>0) && !(name === undefined) && check.length>0)
}
          ...
          <form>
            <div className="form-head">
              <div className="form-boyut">
                <RadioBtn setRadioState = {setRadioState} radioState={radioState} />
              </div>
              <div className="form-hamur">
                <h3 style={{color: (select === '- Hamur Kalınlığı Seç -') && '#ce2829' }}>
                  Hamur Seç{" "}
                  <span style={{ color: "#ce2829", paddingLeft: "5px" }}>
                    *
                  </span>
                </h3>
                <select className="select" value={select} id="hamur-type" name = 'hamurTipi' onChange={(e) => {setSelect(e.target.value)}}>
                  <option value= "- Hamur Kalınlığı Seç -" disabled>- Hamur Kalınlığı Seç -</option>
            ...
            <div className="choise">
              <h3>Ek Malzemeler</h3>
              {check.length > 10 ? <p style={{color:'#ce2829'}}>Daha az malzeme seciniz ( Max 10 ) !</p>  :<p>En Fazla 10 malzeme secçebilirsiniz.5₺</p>}
                ...
                {checkValues.map((item, index) => {
                  return (
                    <>
                      ...
                      />
                      <label key={Date.now()} htmlFor={index}>{item}</label>
                    </>
                  );
                })}
              </div>
            ...
            <div className="form-bottom">
              <input className="not-input" placeholder="Adınız ..." style={{marginTop:'10px'}} onChange={(e) => {setPerName(e.target.value)}}></input>
              {pername.length < 5 &&  <span style={{color: '#ce2829'}}>İsim en az 5 karakter olmalı !</span>}
              ...
              /> 
              {orderNote.length >= 50 && <h3 style={{color:'#ce2829'}}>Max 50 Karakter Giriniz !</h3>}
              <hr style={{ color: "#1a1a1a", opacity: "0.6" }} />
              <div className="total-div">
                <div className="counter-div">
                  <span style={{cursor:'default'}} onClick={orderCountAzalt}>-</span>
                  <span>{orderCount}</span>
                  <span style={{cursor:'default'}} onClick={orderCountArttir}>+</span>
                </div>
                <div className="form-calculate">
                  <div className="calculate-top">
                    ...
                    <div className="secim">
                      ...
                      <span className="secim-span">{((check.length)*5*(orderCount)).toString()+'₺'}</span>
                    </div>
                    <div className="total">
                      <span className="total-spam">Toplam</span>
                      <span className="total-spam">{calculatePrice().toString()+'₺'}</span>
                    </div>
                  </div>
                   <Link to="/order/onay"> 
                    <button disabled = {(submitStatus()) ? false : true} type="submit" onClick={handleSubmitForm}>SİPARİŞ VER</button>
                   </Link>  
                </div>
              </div>
            </div>
